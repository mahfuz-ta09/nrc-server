...
const createAgentRequest = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const {name,email,role,mobile_number,alternate_mobile,dob,address,nationality,passport_number,
        agency_name,agency_address,agency_website,experience,services,partner_universities,license_number,
        tax_id,criminal_record,referral,company_registration_number,company_type,linkedin_profile,additional_notes,
    } = req.body
...
const getAllAgentReq = async(req: AuthenticatedRequest , res: Response) =>{
    try {
        ...
        if (req.query.id) filter._id = new ObjectId(id)
...
const getAllAgents = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        if (applicationStat && applicationStat !== "all") {
            filter.applicationStat = applicationStat
        }
        filter.applicationStat =  "approved"
